...
with st.sidebar:
    st.header("Settings")
    model_path = st.text_input("Model weights (.pt)", value="models/brain_shift_model_02.pt")
...
def load_model(weights: str, device: str = "cpu", explain: bool = False):
    net = (ExplainableUNet3D() if explain else UNet3D()).to(device)
...
def read_minc_stream_to_array(uploaded_file, max_dim: int):
    """Stream an UploadedFile to a temp .mnc, load with nibabel (mmap),
       and coarse-subsample so peak RAM stays small."""
...
def draw_triptych(pre: np.ndarray, post: np.ndarray, heat: np.ndarray, axis: str, idx: int, alpha: float = 0.5):
    ...
    cols = st.columns(3)
    fig1, ax1 = plt.subplots(figsize=(4,4)); ax1.imshow(pre2d, cmap="gray", origin="lower")
    ax1.set_title(f"PRE • {axis} {idx}"); ax1.axis("off"); cols[0].pyplot(fig1); plt.close(fig1)

    fig2, ax2 = plt.subplots(figsize=(4,4)); ax2.imshow(post2d, cmap="gray", origin="lower")
    ax2.set_title(f"POST • {axis} {idx}"); ax2.axis("off"); cols[1].pyplot(fig2); plt.close(fig2)
...
def compute_gradcam(pre: np.ndarray, post: np.ndarray, device: torch.device, modelGC: ExplainableUNet3D):
    """Your Grad-CAM recipe: activations at pooled bottleneck, weighted by grads of mean |flow|."""
    ...
    target = out.norm(p=2, dim=1).mean()          # scalar target
    target.backward()
...
# ---- session state for results (to avoid recompute)
...
if run:
    try:
        ...
        model = load_model(model_path, device_str, explain=want_gradcam) if want_gradcam else load_model(model_path, device_str, explain=False)
        ...
        with st.status("Running inference…", expanded=False) as s:
            mean_mm, max_mm, mag, flow, pre128, post128 = run_inference(pre_arr, post_arr, device, model, target_dim)
            s.update(label="Inference complete.")

        # Optional GT from .tag
        gt_mean = gt_max = None
        ...
        # Store everything for interactive viewing
        ...
        st.session_state.gradcam = None  # reset GCAM cache
        st.success("✅ Done!")
...
if res is not None:
    ...
    with vcol1:
        ...
        heat_source = st.radio("Heatmap", options=["Flow magnitude", "Grad-CAM" if HAS_EXPLAIN else "Flow magnitude"], index=0)
        alpha = st.slider("Overlay alpha", 0.0, 1.0, 0.5, 0.05)

        # compute Grad-CAM once (on demand)
        if heat_source.startswith("Grad") and HAS_EXPLAIN and st.session_state.gradcam is None:
            try:
                device = torch.device(device_str)
                model_g = load_model(model_path, device_str, explain=True)  # ExplainableUNet3D
                with st.spinner("Computing Grad-CAM…"):
                    st.session_state.gradcam = compute_gradcam(res["pre"], res["post"], device, model_g)  # (D,H,W)
    ...
    with vcol2:
        # select heatmap
        heat = res["mag"] if (not HAS_EXPLAIN or heat_source == "Flow magnitude") else (st.session_state.gradcam if st.session_state.gradcam is not None else res["mag"])
        draw_triptych(res["pre"], res["post"], heat, axis, idx, alpha=alpha)

    # export flow
    if want_flow_export:
        with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as tmp:
            np.save(tmp.name, res["flow"])
            path = tmp.name
